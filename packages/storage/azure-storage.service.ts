...
export class AzureStorageService extends StorageService {
    constructor(project: string) {
        super(project);
        this.credentialItemList = ['authId', 'accountId', 'identityId', 'identityToken', 'expiredTime'];
    }

    async hasCachedToken(): Promise<boolean> {
        const identityToken = await this.storage.getItem(`${this.project}.identityToken`);
        const expiredTime = await this.storage.getItem(`${this.project}.expiredTime`);
        return identityToken !== null && expiredTime !== null;
    }
